...
orchestration:
  ...
  quality_gates:
    before_implementation:
      - "Design templates repo cloned and relevant DESIGN.md files read"
      - "2-3 reference brands identified and their patterns absorbed"
...
design_templates_protocol:
  description: >
    Before any frontend layout, UI, site, app, landing page, dashboard or visual component,
    read the design templates from [DESIGN_TEMPLATES_PATH] (cloning if needed from [DESIGN_TEMPLATES_REPO]).
    Use those real-brand patterns as the visual foundation to avoid generic output.
    Templates and agent expertise are COMPLEMENTARY — neither cancels the other.
  brand_selection_approach: >
    List ALL available brands first. Explore beyond the obvious — a SaaS can borrow
    Apple's elegance, a fintech can take Bugatti's precision, a dev tool can use Airbnb's warmth.
    Pick the 2-3 most inspiring brands for THIS specific project, from any category.
    Briefly explain the choice to the user. Unexpected combinations create authentic, memorable design.
  rule: >
    Always scan all available DESIGN.md files before writing any layout code.
    Choose inspiration freely — not by category match, but by creative fit.
    User overrides always take priority — templates are a foundation, not a constraint.
...
core_principles:
  ...
  - "No generic layouts — read real-brand design templates before writing any frontend code"
